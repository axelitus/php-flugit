...
class CloneCommand implements Command
{
    ...
    /**
     * @var string The command's option: destination.
     */
     ...
     */
    public function __construct(Repo $repo, string $source, string $destination = null)
    {
        $this->repo = $repo;
        $this->source($source);
    ...
    public function __toString()
    {
        ...
        // --- destination ---
        $str .= OptionFormatter::formatPath(self::OPTION_DESTINATION, $this->destination);
    ...
    {
        $value !== null && OptionValidator::validNoSpace(self::OPTION_BRANCH, $value);
        $this->branch = $value;
        return $this;
    }
    ...
    {
        $value !== null && OptionValidator::validNoSpace(self::OPTION_CONFIG, $value);
        if ($value === null) {
            $this->configClear();
        } else {
            $this->config[] = $value;
     ...
     * @return CloneCommand Returns itself for method chaining.
     */
    public function destination(string $value = null) : CloneCommand
    {
     ...
     * @return string|null The cloned repository destination.
     */
    ...
    {
        $value !== null && OptionValidator::validNoSpace(self::OPTION_ORIGIN, $value);
        $this->origin = $value;
        return $this;
    }
